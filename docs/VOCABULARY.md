...
| `preimage_fiber_profile` | preimage-fiber profile | state-merging atlas | The ordered target-side fiber data showing how many source states collapse onto each observed target state on a selected finite window. | all coprime moduli |
| `visible_preimage_compression` | visible preimage compression | visible obstruction | A quotient-only selected-coordinate obstruction where multiple remainder states already collapse onto one carry state on the aligned finite window. | all coprime moduli |
| `hidden_graph_obstruction` | hidden graph obstruction | hidden obstruction | A quotient-only selected-coordinate obstruction where the aligned finite window is bijective but relabeling still fails because the carry and remainder graph layers have incompatible observed sizes. | all coprime moduli |
